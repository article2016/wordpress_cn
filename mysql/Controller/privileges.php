...
{
	public $indexs = null;
	public $MysqlFetchType = MYSQL_ASSOC;		// 查询数据FetchType
	...
	function IndexAction()
	{
		...
		$sql = 'SELECT `User`, `Host`, `Password`, `max_questions`, `max_updates`, `max_connections`, `max_user_connections` FROM mysql.user';
		$UserData = $this -> indexs -> GetSqlData($sql, null, null, $this -> QueryFrom, array(0, 0), $this -> MysqlFetchType);
		
		$NewName = array('User', 'Host', 'Pass', 'Database', 'Table', 'Grant', 'Privileges', 'max_questions', 'max_updates', 'max_connections', 'max_user_connections');
		$name_list = array();
